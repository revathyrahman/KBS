...
namespace LCI.QualityTools.BrowserTests.MyLCI
{
    public class UtilityMethods
    {
        ...
        public Boolean LoginMyLCI(String Role, String UserId, String Password)
        {
            ...
            {
                // Enter user name
                EnterValueById(PageObjects.idLoginUserName, UserId, "Login User ID");

                // Enter password
                EnterValueById(PageObjects.idLoginPassword, Password, "Login Password");

                // Click login button
                ClickById(PageObjects.btnLogin, "Login button");
        ...
        public Boolean VerifyAddClubLinkExists(String LinkText)
        {
            ...
            try
            {
                ...
                if (browserDriver.FindElement(By.LinkText(PageObjects.linkTextAddClub)).Displayed)
                {
                    flag = true;
        ...
        public Boolean FindClubs()
        {
            ...
            try
            {
                if (browserDriver.FindElement(By.Name(PageObjects.mnuMyDistrict)).Displayed)
                {
                    browserDriver.FindElement(By.Name(PageObjects.mnuMyDistrict)).Click();
                }
                else
                {
                    browserDriver.FindElement(By.Id(PageObjects.mnuLinkDistricts)).Click();
                }
                browserDriver.FindElement(By.Name(PageObjects.mnuNameMyDistrictClubs)).Click();
        ...
        /// </summary>
        /// <remarks>
        /// Takes three arguments id ,visibletext and fielddesc returns true on selection of value in dropdown list and returns false  if it doesn't
        /// </remarks>
        ...
        /// <summary>
        /// This method is to click based on Name of the element
        /// </summary>
        /// <remarks>
        /// Takes two argument Name and fielddesc and returns true on element is clicked and false if it doesn't
        /// </remarks>

        public void ClickByName(String elementName, string fieldDesc)
        {
            Boolean flag = true;
            try
            {
                browserDriver.FindElement(By.Name(elementName)).Click();
                excelReporter.ReportStep("Element with ID :" + elementName + "of " + fieldDesc + " is found and clicked successfully..", "SUCCESS");
            }
            catch (NoSuchElementException e)
            {
                excelReporter.ReportStep("Element with ID :" + elementName + "of " + fieldDesc + "could not be found..", "FAILURE");
                flag = false;
            }
            catch (WebDriverException exe)
            {
        ...
        public string AddClubFormEntry()
        {
            ...
            try
            {
                ...
                EnterValueById(PageObjects.idClubName, ClubName, "Club Name");

                //Select Club Type
                SelectDropdownValueByVisibleText(PageObjects.ddlClubType, "Lions Club", "Club Type");

                //Enter Club City 
                EnterValueById(PageObjects.idClubCity, "Automation TestCity", "Club City");


                //Select from Club Language
                SelectDropdownValueByVisibleText(PageObjects.ddlclubLang, "English", "Club Language");

                // Verify the list of club languages in the dropdown
                VerifyLanguageListAddClub();

                //Click for a sponsoring club
                ClickById(PageObjects.btnSponsoringClub, "Sponsoring ClubName");
                browserDriver.Manage().Timeouts().ImplicitlyWait(TimeSpan.FromSeconds(5));
                ClickByXPath(PageObjects.xpathListSponsoringClubs, "Sponsoring Club");
                browserDriver.Manage().Timeouts().ImplicitlyWait(TimeSpan.FromSeconds(3));

                //Enter New Club President creation details
                ClickByXPath(PageObjects.xpathClubPresidentPanel, "New Club President Creation Panel");
                EnterValueById(PageObjects.idPresidentFirstname, "PresidentFirstname", "New Club President FirstName");
                EnterValueById(PageObjects.idPresidentLastname, "PresidentLastname", "New Club President LastName");
                EnterValueById(PageObjects.idPresidentYOB, "1980", "New Club President YOB");
                SelectDropdownValueByVisibleText(PageObjects.idPresidentGender, "Male", "New Club President Gender");
                EnterValueById(PageObjects.idPresidentEmailaddress, "president@test.com", "New Club President Email");

                //Enter New Club Secretary Creation details
                ClickByXPath(PageObjects.xpathClubSecretaryPanel, "New Club Secretary Creation Panel");
                EnterValueById(PageObjects.idSecretaryFirstname, "SecretaryFirstName", "New Club Secretary Firstname");
                EnterValueById(PageObjects.idSecretaryLastname, "SecretaryLastName", "New Club Secretary LastName");
                EnterValueById(PageObjects.idSecretaryYOB, "1980", "New Club Secretary YOB");
                SelectDropdownValueByVisibleText(PageObjects.idSecretaryGender, "Female", "New Club Secretary Gender");
                EnterValueById(PageObjects.idSsecretaryEmailaddress, "testsecretary@test.com", "New Club Secretary Email");

                //Enter Charter Member details
                EnterValueById(PageObjects.idNewMemberscount, "20", "New Member Count");
                EnterValueById(PageObjects.idTransferMemberscount, "0", "Transfer Member Count");
                EnterValueById(PageObjects.idStudentMemberscount, "0", "Student Member Count");
                EnterValueById(PageObjects.idLeoLionscount, "0", "LeoLions Member Count");

                //Check New Club Criteria checkbox
                ClickById(PageObjects.idNewClubCriteria, "New Club Criteria");
                //Enter Comment
                EnterValueById(PageObjects.idComments, "test comment", "Enter Comment");

                //Click on Save
                ClickById(PageObjects.idBtnSave, "Save Button");
        ...
        public void DiscontinueClub(string ClubName)
        {
            try
            {
                ClickById(PageObjects.idDiscontinueClubchkbox, "Discontinue Club Checkbox");
                EnterValueById(PageObjects.idDiscontinueNote, "Test Comment", "Comment");
                ClickById(PageObjects.idBtnSave, "Save Button");
        ...
        public void ContinueClub(string ClubName)
        {
            ...
            try
            {
                ClickById(PageObjects.idContinueClub, "Continue Club checkbox");
                // EnterValueById(PageObjects.idDiscontinueNote, "Test Comment", "Continue club Comment");
                ClickById("btnSave", "Save Button");
                string ConfirmationMessage = browserDriver.FindElement(By.XPath(PageObjects.xpathClubConfirmationMsg)).Text;
            ...
            try
            {
                switch (currentStatus)
                {
                    case ("Pending Submission"):
                        ClickById(PageObjects.idNewClubCriteria, "New Club Criteria");
                        ClickById(PageObjects.idBtnSubmitPendingSubmission, "DG Auth Submit button");
                        Thread.Sleep(5000);
                        ClickById("btnSave", "Save Button");
                        Thread.Sleep(5000);
                        confirmationMessage = browserDriver.FindElement(By.XPath(PageObjects.xpathClubConfirmationMsg)).Text;
                        if (confirmationMessage.Contains("District Governor authorization"))
                        {
                            excelReporter.ReportStep("Club application moved to Pending DG/CL Authorization status", "Pass");
                        }
                        else
                        {
                            flag = false;
                            excelReporter.ReportStep("Club application not moved to Pending LCI Authorization status", "False");
                        }
                        break;

                    case ("DG/CL Authorization"):
                        ClickById(PageObjects.idNewClubCriteria, "New Club Criteria");
                        ClickById(PageObjects.idBtnSubmitPendingDGAuth, "DG Auth Submit button");
                        Thread.Sleep(5000);
                        ClickById("btnSave", "Save Button");
                        Thread.Sleep(5000);
                        confirmationMessage = browserDriver.FindElement(By.XPath(PageObjects.xpathClubConfirmationMsg)).Text;
                        if (confirmationMessage.Contains("District Governor authorization"))
                        {
                            excelReporter.ReportStep("Club application moved to Pending LCI Authorization status", "Pass");
                        }
                        else
                        {
                            flag = false;
                    ...
                    case ("LCI Authorization"):
                        ClickById(PageObjects.idNewClubCriteria, "New Club Criteria");
                        ClickById(PageObjects.idBtnSubmitPendingLCIAuth, "LCI Auth Submit button");
                        Thread.Sleep(5000);
                        ClickById("btnSave", "Save Button");
                        Thread.Sleep(5000);
                        confirmationMessage = browserDriver.FindElement(By.XPath(PageObjects.xpathClubConfirmationMsg)).Text;
                        if (confirmationMessage.Contains("LCI authorization"))
                        {
                            excelReporter.ReportStep("Club application moved to Pending Club completion status", "Pass");
                        }
                        else
                        {
                            flag = false;
                            excelReporter.ReportStep("Club application not moved to Pending Club completion status", "False");
                        }
                        break;
        ...
        public void FindDesiredClub(String Filtername)
        {
            ...
            try
            {
                ...
                ClickById(PageObjects.lblSearchOption, "FindClub Search List");
        ...
        public void NewClubOfficersChk()
        {
            ...
            try
            {
                ...
                ClickByXPath(PageObjects.xpathClubPresidentPanel, "President Header");
                VerifyFieldEdit("First Name", PageObjects.idPresidentFirstname);
                VerifyFieldEdit("Last Name", PageObjects.idPresidentLastname);
                VerifyFieldEdit("Year Of Birth", PageObjects.idPresidentYOB);
                ...
                ClickByXPath(PageObjects.xpathClubSecretaryPanel, "Secretary Header");
        ...
        public void ECMForLeoLionsClub()
        {
            ...
            try
            {
                ...
                VerifyFieldEdit("Transfer Members", PageObjects.idTransferMemberscount);
                VerifyFieldEdit("Student Members", PageObjects.idStudentMemberscount);
                VerifyFieldEdit("Young Adults", PageObjects.idYoungAdultscount);
                VerifyFieldEdit("Leo Lions", PageObjects.idLeoLionscount);
        ...
        /// </summary>
        ///<remarks>
        /// Takes two arguments ID and text returns nothing
        /// </remarks>
        ...
        public void VerifyMessageclubcount()
        {
            ...
            try
            {
                ...
                {
                    flag = true;
                    ClickById(PageObjects.idAddClub, "Add Club");
